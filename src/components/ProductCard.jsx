...
const ProductCard = ({ product }) => { // 1. On ne reçoit que l'objet complet
  const [isModalOpen, setIsModalOpen] = useState(false);
  const { addToCart, toggleFavorite, favorites } = useContext(GlobalContext);
  
  // Sécurité : si product est vide, on arrête tout
    ...
    <>
      <div className="bg-neutral-900 rounded-2xl p-4 border border-white/5 group hover:border-[#00A3FF] transition-all">
        ...
        {/* <button onClick={() => setIsModalOpen(true)}
          className="w-full mt-3 py-3 bg-[#00A3FF] text-white font-bold rounded-lg hover:bg-[#008bd9] transition">
           Commander
        </button> */}
      </div>
